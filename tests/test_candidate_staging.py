...
# ------------------------------------------------- blank and padded names
#
# The escape, extension and HTTPS cases are covered above. These pin what is
# left: how a name that is merely untidy, rather than dangerous, is treated.
...
def test_a_blank_filename_falls_back_to_the_downloaded_name(
    ...
    An empty string used to fall back while a whitespace-only one was refused —
    the same intent taking two different paths.
    """
    staged = stage_candidate(
...
def test_the_name_is_checked_before_the_file_is_read(tmp_path: Path, download: Path) -> None:
    ...
    with pytest.raises(CandidateError, match="plain basename"):
        stage_candidate(
            download,
            tmp_path / "candidates",
            CLEARED,
