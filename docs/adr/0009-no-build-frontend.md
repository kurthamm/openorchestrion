# ADR-0009: The web application ships as source, with no frontend build step
...
The obvious default — React with Vite, or an equivalent bundler — would add a
Node toolchain to a project that is otherwise pure Python, and would add a build
...
thin client, not a client-side application with its own domain model — the
workload a heavy framework exists to manage largely does not exist here.

Chromium on Raspberry Pi OS supports ES modules, `import`, CSS custom
properties, container queries, and `crypto.randomUUID()` natively, so "modern
syntax needs transpiling" no longer applies to the one browser the appliance
must run.
...
- Keep DOM rendering in small pure-ish render functions driven by a single store.

## Consequences

- `git clone` plus `pip install -e .` produces a running appliance, with no
  second toolchain and no build artifact to keep in sync.
- CI needs no Node job; the existing Python workflow covers the whole product.
...
- No JSX, no TypeScript, and no npm ecosystem. Component composition is manual,
  and type safety across the API boundary comes from generating against
  `/openapi.json` rather than from a compiler.
- If the administration surface later grows genuinely application-like
  complexity — multi-step editors, offline queues of pending edits — this
  decision should be revisited for that surface specifically rather than
  abandoned wholesale. Adding a build step later is a contained change, because
  view code is already isolated behind the store and API modules.
